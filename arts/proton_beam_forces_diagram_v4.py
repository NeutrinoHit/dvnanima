...
p1 = np.array([-1.65, 0.45])
p2 = np.array([ 1.05, 0.45])
...
def stroke_text(txt, lw=3.2):
    txt.set_path_effects([pe.withStroke(linewidth=lw, foreground="white")])
    return txt

ax.add_patch(Circle((0, -0.1), 4.95, color="#f7f7fb", zorder=-10))
ax.add_patch(Circle((0, -0.1), 4.45, fill=False, lw=0.8, color="#e1e4ef", zorder=-9))
...
def circle_with_tangent_arrows(center, radius, color, label, label_xy, thetas, lw=3.0):
    ...
    stroke_text(ax.text(*label_xy, label, fontsize=18.5, fontweight="bold",
                        color=color, ha="center", va="center", zorder=9))

circle_with_tangent_arrows(p1, R, blue1, r"$\mathbf{B}_1$", (-3.45, 2.95),
                           thetas=[0.03, 2.35, 5.45])
circle_with_tangent_arrows(p2, R, blue2, r"$\mathbf{B}_2$", (2.90, 2.95),
                           thetas=[np.pi-0.03, 0.78, 3.95])

def velocity_line(p):
    start = p + np.array([0.0, 0.48])
    end = p + np.array([0.72, 3.20])
    ...
    ax.add_patch(FancyArrowPatch(start + 0.62*(end-start), end,
                                 ...
                                 color=gray, zorder=4))
    stroke_text(ax.text(end[0]+0.16, end[1]+0.12, r"$\mathbf{v}$",
                        fontsize=19, color=black, fontweight="bold",
                        ha="center", va="center", zorder=10))

velocity_line(p1)
velocity_line(p2)

ax.plot([p1[0] + 0.55, p2[0] - 0.55], [p1[1], p1[1]],
        color="#999999", lw=1.5, linestyle=(0, (4, 5)), zorder=0)
stroke_text(ax.text((p1[0]+p2[0])/2, p1[1]-0.24, r"$r$", fontsize=17,
...
stroke_text(ax.text(0, 5.00, "Силы между двумя протонами в пучке",
                    fontsize=22, fontweight="bold", ha="center", color=black, zorder=20))

panel_x, panel_y = 0.55, -0.55
panel_w, panel_h = 4.35, 4.45
panel = FancyBboxPatch((panel_x, panel_y-panel_h), panel_w, panel_h,
                       boxstyle="round,pad=0.18,rounding_size=0.12",
                       fc="white", ec="#d8d8e5", lw=1.2, alpha=0.96, zorder=13)
ax.add_patch(panel)

stroke_text(ax.text(panel_x+0.18, panel_y-0.35, "Быстрый вывод",
                    fontsize=16.5, fontweight="bold", color=black,
                    ha="left", va="center", zorder=20), lw=2.5)

deriv_lines = [
    r"1. $\mathbf{F}=q(\mathbf{E}+\mathbf{v}\times\mathbf{B})$",
    r"2. $\mathbf{F}_e=q\mathbf{E}$, $\;\mathbf{F}_m=q\,\mathbf{v}\times\mathbf{B}$",
    r"3. Для поля движущегося заряда: $\;B \simeq \dfrac{vE}{c^2}$",
    r"4. Тогда $\;F_m=qvB \simeq qv\dfrac{vE}{c^2}=\beta^2 qE=\beta^2 F_e$",
    r"5. $\mathbf{F}_m$ направлена против $\mathbf{F}_e$",
    r"6. Поэтому $\;F_{\rm res}=F_e-F_m=F_e(1-\beta^2)$",
    r"7. Так как $\;\gamma=\dfrac{1}{\sqrt{1-\beta^2}}$, то $\;1-\beta^2=\dfrac{1}{\gamma^2}$",
    r"8. Итого: $\;F_{\rm res}= \dfrac{F_e}{\gamma^2}$"
]
y0 = panel_y - 0.80
dy = 0.47
for i, line in enumerate(deriv_lines):
    ax.text(panel_x+0.20, y0 - i*dy, line,
            fontsize=11.4 if i != 7 else 12.6,
            color=black, ha="left", va="center", zorder=20)

box_y = -4.55
box = FancyBboxPatch((-2.25, box_y - 0.28), 4.50, 0.72,
                     boxstyle="round,pad=0.10,rounding_size=0.12",
                     fc="white", ec="#d8d8e5", lw=1.2, alpha=0.95, zorder=14)
ax.add_patch(box)
ax.text(0, box_y + 0.03,
        r"$|\mathbf{F}_{\rm res}|\simeq F_e(1-\beta^2)=F_e/\gamma^2$",
        fontsize=18, ha="center", va="center", color=black, zorder=20)

ax.text(-5.00, -5.10, r"$\mathbf{F}_e$ — электрическое отталкивание",
        fontsize=12.8, color=red, ha="left", zorder=20)
ax.text(-5.00, -5.42, r"$\mathbf{F}_m$ — магнитное притяжение",
        fontsize=12.8, color=green, ha="left", zorder=20)
ax.text(0.55, -5.10, r"$\mathbf{B}_1,\mathbf{B}_2$ — магнитные поля протонов",
        fontsize=12.8, color=blue1, ha="left", zorder=20)
ax.text(0.55, -5.42, r"$\mathbf{v}$ — скорость протонов",
        fontsize=12.8, color=black, ha="left", zorder=20)

plt.savefig("proton_beam_forces_diagram_v4.png", dpi=220, facecolor="white")
plt.savefig("proton_beam_forces_diagram_v4.svg", facecolor="white")
plt.show()
